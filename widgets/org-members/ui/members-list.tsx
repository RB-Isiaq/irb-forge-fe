...
const roleBadgeVariant: Record<OrgRole, "primary" | "success" | "default" | "outline"> = {
  owner: "primary", admin: "success", mentor: "default", member: "outline",
};
...
export function MembersList({ slug }: { slug: string }) {
  ...
  return (
    <div className="rounded-[12px] border border-border bg-surface overflow-hidden">
      {members.map((m, i) => (
        <div key={m.id} className={`flex items-center gap-3 px-5 py-3.5 ${i < members.length - 1 ? "border-b border-border" : ""}`}>
          <Avatar firstName={m.user.firstName} lastName={m.user.lastName} size="sm" />
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-medium text-text-primary">{m.user.firstName} {m.user.lastName}</p>
